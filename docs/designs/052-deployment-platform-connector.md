...
- Every batch is pinned to its caller token's node claim: a token minted on node X carries authority over node X only, wherever it is presented.
...
- The write path gains one network hop of latency. The budget: a cache-hit request adds sub-millisecond server work, a miss one TokenReview round trip of a few milliseconds, against an RPC timeout of 10 seconds; this is confirmed by a load test at the modeled rates rather than assumed.
