...
These settings apply to the node-local Kubernetes queue. On the deployment platform connector the Kubernetes connector runs inside the request as best effort within `ConditionUpdateTimeout`; a failure or timeout is counted in `platform_connector_best_effort_failures_total{connector="kubernetes"}` and the batch is acknowledged anyway.
