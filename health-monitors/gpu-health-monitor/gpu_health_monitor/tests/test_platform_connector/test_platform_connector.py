...
class TestPlatformConnectors(unittest.TestCase):
    ...
    def test_blocked_publish_serializes_polls(self) -> None:
        """While the first fault publish is blocked (an outage in direct mode), a repeated fault poll and a later
        healthy poll wait their turn under the event lock; when publishing resumes the repeated fault finds the
        cache already published and the recovery is published, so the fault goes out once and the cache ends
        healthy."""
        ...
        try:
            ...
            polls.append(Thread(target=processor.health_event_occurred, args=(healthy, [0])))
            for poll in polls[1:]:
                poll.start()
            # Both later polls are queued on the lock behind the blocked publish.
            assert wait_until(lambda: all(poll.is_alive() for poll in polls[1:])), "the later polls are waiting"
            release.set()
            for poll in polls:
                poll.join(10.0)
            ...
            assert pcie_sends == [[False], [True]], "the fault once, then the recovery: %r" % (sends,)
