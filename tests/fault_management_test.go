...
func TestHealthyEventCancelsActiveDrain(t *testing.T) {
	feature := features.New("TestHealthyEventCancelsActiveDrain").
		WithLabel("suite", "fault-management-healthy-recovery")
	...
	feature.Assess("send healthy event before deleteAfterTimeout expires", func(ctx context.Context, t *testing.T, c *envconf.Config) context.Context {
		t.Log("Sending healthy event to trigger recovery")
	...
	feature.Assess("verify FQ unquarantines the node", func(ctx context.Context, t *testing.T, c *envconf.Config) context.Context {
		client, err := c.NewClient()
		require.NoError(t, err)

		t.Log("Waiting for FQ to unquarantine node")
	...
	feature.Assess("verify ND stops draining (label removed)", func(ctx context.Context, t *testing.T, c *envconf.Config) context.Context {
		...
		t.Log("Waiting for ND to process UnQuarantined event and remove draining label")
		require.Eventually(t, func() bool {
			node, err := helpers.GetNodeByName(ctx, client, testCtx.NodeName)
			if err != nil {
				return false
...
func TestHealthyEventWithFaultRemediation(t *testing.T) {
	feature := features.New("TestHealthyEventWithFaultRemediation").
		WithLabel("suite", "fault-management-healthy-recovery-fr")
	...
	feature.Assess("create pods and trigger quarantine", func(ctx context.Context, t *testing.T, c *envconf.Config) context.Context {
		...
		t.Log("Sending fatal health event to trigger quarantine and remediation")
		fatalEvent := helpers.NewHealthEvent(testCtx.NodeName).
			WithErrorCode("79").
			WithMessage("XID 79 fatal error").
			WithRecommendedAction(24)
	...
	feature.Assess("send healthy event", func(ctx context.Context, t *testing.T, c *envconf.Config) context.Context {
		t.Log("Sending healthy event to trigger recovery")
		healthyEvent := helpers.NewHealthEvent(testCtx.NodeName).
			WithErrorCode("79").
			WithHealthy(true).
	...
	feature.Assess("verify FR clears remediation state annotation", func(ctx context.Context, t *testing.T, c *envconf.Config) context.Context {
		...
		t.Log("Waiting for FR to process UnQuarantined event and clear remediation state")
		require.Eventually(t, func() bool {
			node, err := helpers.GetNodeByName(ctx, client, testCtx.NodeName)
			if err != nil {
				return false
