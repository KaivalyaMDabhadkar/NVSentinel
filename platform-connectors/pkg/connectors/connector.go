...
// their result.
//
// Set hands one batch to several connectors and fails with the first
// failure. BestEffort wraps a connector whose failure must not fail the
// batch. Which connector decides the reply is therefore visible where the
// set is built: the member that is not wrapped.
package connectors
...
// them. It fails with the first member's error, and that failure cancels the
// context the other members run on, so they stop early instead of finishing
// work on a batch that will be resent. A member whose failure must not fail
// the batch is wrapped in BestEffort.
type Set []Connector
...
func (s Set) ProcessBatch(ctx context.Context, he *pb.HealthEvents) error {
	...
	for _, c := range s {
		wg.Go(func() {
			err := c.ProcessBatch(memberCtx, he)
			if err == nil {
				return
			}
...
// anyway. It is for work that repairs
// itself, like node conditions the next report rewrites, and for forwards
// nobody waits for. When the caller's own context ends first, the connector
// was cut short with the request and nothing is counted: the batch is not
// acknowledged and will be resent.
...
func (b *bestEffort) ProcessBatch(ctx context.Context, he *pb.HealthEvents) error {
	...
	if ctx.Err() != nil {
		// The request ended or another member failed: the batch is not
		// acknowledged and will be resent, so this is not a failure of ours.
